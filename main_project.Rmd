...
- Maybe before moving to counts_filtered and further proccesing check if the data makes sense. For instance:
  - Is dox regulating any genes (such as GFP or other reporter) that we can verify the increased expression (higher counts) for later time points? Verify this is the case in the counts_orig df. 
  - For the previous question, we could even consider making a simple ggplot showing the expression of the reporter for all time points. May be also easier to do for the tpm files.
  
- For the filtering, look more into what other people do or try different parameters
  - I have seen filtering by rowSums(counts_matrix) > 10
  - Also, I have seen filtering done at the dds (DESeqDataSetFromMatrix) object instead of filtering before. Maybe see what is the difference?
