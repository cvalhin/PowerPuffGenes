...
GTF_PATH         <- "/scratch/Shares/rinnclass/MASTER_CLASS/GENOMES/M25/gencode.vM25.annotation.gtf"
FILTER_TRESHOLD  <- 0
P_VALUE_FILTER   <- 0.05
LOG2_FOLD_FILTER <- 1
save(COUNTS_PATH, TPM_PATH, GTF_PATH, FILTER_TRESHOLD, P_VALUE_FILTER, LOG2_FOLD_FILTER, file = "results/parameters.RData")
```
...
``` r
# Create deseq_samples and extract metadata
deseq_samples <- data.frame(sample_id = colnames(counts)) %>%
  mutate(
    time_point = sapply(strsplit(sample_id, "_"), function(x) x[[2]]),
    replicate = sapply(strsplit(sample_id, "_"), function(x) x[[3]]),
    time_point = factor(time_point, levels = c("0", "12", "24", "48", "96")),
    replicate = factor(replicate)
  )

# Verify sample names match
stopifnot(all(colnames(counts) == deseq_samples$sample_id))
...
dds <- DESeqDataSetFromMatrix(countData = counts_filtered,
                              ...
                              design = ~ time_point) 
dds <- DESeq(dds)
#save(deseq_samples, dds, file = "results/deseq_files.RData")
```
...
<figure>
<img src="results/pca_plot.png" alt="PCA Plot" />
...
avg_and_sd_values <- time_points %>%
  map_dfc(function(tp) {
    # Find columns for this time point
    cols <- grep(paste0("WT_", tp, "_"), colnames(tpm_filtered), value = TRUE)
...
plot_count_distributions(
  filtered_results,
  output_file = "results/counts_distributions.png"
)

# Use the function with your avg_and_sd_values
plot_tpm_distributions(
  avg_sd_df = avg_and_sd_values,
  output_file = "results/tpm_distributions.png"
)
```

![DESEQ Distributions](results/counts_distributions.png) ![TPM
Distributions](results/tpm_distributions.png)
...
plot_baseline <- create_volcano_plot(
  ...
  plot_title = "Volcano Plots: Comparisons vs Baseline",
  output_file = "results/volcano_baseline.png"
)
...
plot_timeseries <- create_volcano_plot(
  ...
  plot_title = "Volcano Plots: Sequential Time Comparisons",
  output_file = "results/volcano_timeseries.png"
)
```

![Volcano Baseline Plot](results/volcano_baseline.png) ![Volcano
Timeseries Plot](results/volcano_timeseries.png)
...
# Save gene groups to RData file
#save(
#  change_at_12_to_end = gene_groups$change_at_12_to_end,
#  change_at_24_to_end = gene_groups$change_at_24_to_end,
#  change_at_48_to_end = gene_groups$change_at_48_to_end,
#  transient_genes = gene_groups$transient_genes,
...
lfc_plot <- plot_lfc_prolonged(
  results, 
  gene_groups,
  output_file = "results/lfc_prolonged_genes.png"
)
...
tpm_plot <- plot_tpm_prolonged(
  ...
  g2s, 
  filtered_results,
  output_file = "results/tpm_prolonged_genes.png"
)
```

![LFC Prolongued Genes](results/lfc_prolonged_genes.png) ![TPM
Prolongued Genes](results/tpm_prolonged_genes.png)
...
all_genes_heatmap <- plot_gene_heatmap(
  ...
  title = "Heatmap of Log2-Transformed TPM Values for All Significant Genes",
  show_gene_names = FALSE,
  output_file = "results/heatmap_all_genes.png"
)
...
prolonged_genes_heatmap <- plot_gene_heatmap(
  ...
  title = "Heatmap of Log2-Transformed TPM Values of Prolonged Genes",
  show_gene_names = TRUE,
  output_file = "results/heatmap_prolonged_genes.png"
)
```

<figure>
<img src="results/heatmap_all_genes.png" alt="Heatmap of All Genes" />
<figcaption aria-hidden="true">Heatmap of All Genes</figcaption>
</figure>

![Heatmap of Prolongued Genes](results/heatmap_prolonged_genes.png) \###
Circos Plot
...
gene_locations <- prepare_gene_annotations(
  gtf_file = GTF_PATH,
  valid_chromosomes = paste0("chr", c(1:19, "X", "Y"))  # Mouse chromosomes
)
...
<figure>
<img src="results/circos_differential_expression.png"
alt="Circos Plot - Baseline Comparisons" />
<figcaption aria-hidden="true">Circos Plot - Baseline
