...
library(circlize)
library(stringr)
source(file = "utils/proccess_files_test2.R")
source(file = "utils/plots_test2.R")
```
...
P_VALUE_FILTER   <- 0.01
LOG2_FOLD_FILTER <- 1
save(COUNTS_PATH, TPM_PATH, GTF_PATH, FILTER_TRESHOLD, P_VALUE_FILTER, LOG2_FOLD_FILTER, file = "results/r_parameters.RData")
```
...
save(results, filtered_results, filtered_genes, file = "results/r_results_counts.RData")
```
...
# Save results
save(avg_and_sd_values, file = "results/r_results_tpm.RData")
```
...
```
![DESEQ Distributions](results/counts_distributions.png)
![TPM Distributions](results/tpm_distributions.png)
...
```
![Volcano Baseline Plot](results/volcano_baseline.png)
![Volcano Timeseries Plot](results/volcano_timeseries.png)
...
#  file = "results/r_gene_groups.RData"
#)
...
![LFC Prolongued Genes](results/lfc_prolonged_genes.png)
![TPM Prolongued Genes](results/tpm_prolonged_genes.png)
...
![Heatmap of All Genes](results/heatmap_all_genes.png)
...
![Circos Plot - Baseline Comparisons](results/circos_differential_expression.png)
